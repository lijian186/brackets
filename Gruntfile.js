...
angular.module('adf.widget.tox', ['adf.provider'])
  ...
  .controller('toxCtrl', ["$http", "$q", "$scope", "$interval", "config", "commonService", function ($http, $q, $scope, $interval, config, commonService) {
    ...
    $scope.startTox = function(){
      $http.get('http://'+$scope.microtool.extUrls[0] + '/tox').then(function(exitCodeResponse){
      }, function(err){
      });
    };
...
$templateCache.put("{widgetsPath}/tox/src/view.html","<div><div ng-show=microtool><p ng-hide=microtool.extUrls[0]>Cannot get the link to Tox.</p><a target=_blank class=btn ng-if=microtool.extUrls[0] ng-click=startTox()>Start Tox</a><br><div ng-if=details class=microtool><div ng-if=details.message><span class=\"glyphicon glyphicon-ok-circle\"></span><p class=length>{{ details.message }} at {{details.timeStamp}}</p></div><div ng-if=details.error><span class=\"glyphicon glyphicon-remove-circle\"></span><p>{{ details.error }} at {{details.timeStamp}}</p></div></div><div ng-if=!details><p><b>Tox</b> microtool is not running.</p></div></div><div ng-hide=microtool><p>No Tox Microtool</p></div></div><style>\n  p.padding-5-margin-0{\n    padding-top: 5px;\n    margin: 0;\n  }\n  .glyphicon{\n    padding-right: 10px;\n  }\n  .failure-code {\n    margin: 0 !important;\n    padding: 5px 25px !important;\n  }\n  .microtool .glyphicon-ok-circle{\n    color: #00cc55;\n  }\n  .microtool .glyphicon-remove-circle{\n    color: #ee9999;\n  }\n  .microtool .glyphicon-warning-sign{\n    color: #eeee55;\n  }\n  .length{\n    word-wrap:break-word; width:auto;\n  }\n</style>");}]);})(window);
